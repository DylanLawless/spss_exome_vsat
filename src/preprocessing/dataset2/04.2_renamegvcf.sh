...
# sbatch --array=0-379 04.2_renamegvcf.sh
module load picard/2.20.8

# Path: temp work
SCRATCH_DIR="/scratch/lawless/spss_exome"
INPUT_DIR="${SCRATCH_DIR}/haplotype_caller_output_temp"
OUTPUT_DIR="${SCRATCH_DIR}/haplotype_caller_output_rename"

# Declare
declare -a total_file
for i in ${INPUT_DIR}/*.g.vcf; do total_file+=($i); done
filebase=${total_file[$SLURM_ARRAY_TASK_ID]}
sample_id=$(basename ${filebase} .g.vcf)

picard RenameSampleInVcf \
INPUT=${INPUT_DIR}/${sample_id}.g.vcf \
OUTPUT=${OUTPUT_DIR}/${sample_id}_rename.g.vcf \
NEW_SAMPLE_NAME=$sample_id

# check names
perl /work/gr-fe/lawless/tool/vcfhacks/getSampleNames.pl -i ${OUTPUT_DIR}/${sample_id}_rename.g.vcf
