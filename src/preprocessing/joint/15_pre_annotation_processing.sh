...
# Path
INPUT_DIR=${WORK_DIR}/genotype_refinement_output
OUTPUT_DIR=${WORK_DIR}/pre_annotation_output
...
#echo "$(date)"
## Exclude filtered
#$GATK --java-options "-Djava.io.tmpdir=${SCRATCH_DIR}/temp/${SLURM_JOBID}/io -Xms16G -Xmx16G -XX:ParallelGCThreads=4" \
#        SelectVariants \
...
#echo "$(date)"
#$VCFTOOLS --vcf bcftools_gatk_norm.vcf \
#	--max-maf 0.01 \
#	--recode --recode-INFO-all \
#	--out bcftools_gatk_norm_maf01
## Exclude on minor allele frequency. max-maf 0.01*940 samples = max 9.4 alleles (<5 hom or <10 het)

printf "\nfinal bgzip and index tbi\n"
echo "$(date)"
bgzip -c bcftools_gatk_norm_maf01.recode.vcf > bcftools_gatk_norm_maf01.recode.vcf.gz
tabix -p vcf bcftools_gatk_norm_maf01.recode.vcf.gz
rm temp.vcf
rm bcftools_gatk_norm_maf01.recode.vcf
