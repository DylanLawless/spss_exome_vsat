...
# Note our sample set is already pre-defined - the others are for testing different ideas about QV as we explored in Manon's MSc project
...
if [[ -n "${Subset_resampling}" ]]; then
    ...
    file_suffix="subset_ppi_whole_genome_interpret_skat_${version_suffix}"
    ...
    file_suffix="ppi_whole_genome_interpret_skat_${version_suffix}"
    output_param=""
...
# template used to create a temporary job script
# note that i remove the "%a" from the log name since the current_MCLID is not 
# read from SLURM_ARRAY_TASK_ID until later. The err/out log would be labelled 
# "1,2,3,..." instead of the MCL_ID and possibly misleading.
...
#SBATCH --job-name=ppi_${version_suffix}
#SBATCH --output=./log/out_err_${version_suffix}/${output_param}skat_ppi_%J.out
#SBATCH --error=./log/out_err_${version_suffix}/${output_param}skat_ppi_%J.err
