...
sink(paste("log/log_", version_suffix, "/log_skat_", file_suffix, "_MCL_", current_MCLID, ".log", sep = ""), append = TRUE)
...
# covar
X2 <- read.csv("../../data/joint/covar/joint.csv",
				  sep = ",",
				  header = F)
...
clusters_HIGH_joint$MCL_ID <- clusters_HIGH_joint$ID
...
# 90% genotyping per variant was sufficient in testing.

# genotype sum
# df_AN <- df %>%
# 	group_by(SNP, genotype) %>%
# 	summarise(n_geno = n()) %>% # if message ends up in output, suppress with: .groups = "drop"
...
cat("of the total MCL genes:", current_SYMBOL_length, "\n")
...
# the "OK" name is from testing to indicate results that were "OK" - should be renamed
# loop over unique MCL_IDs
skat_run <- for (i in 1:length(unique_symbols)) {
	
	# filter for the current ~SYMBOL~ MCL_ID
	OK_filtered <- OK |> filter(MCL_ID == unique_symbols[i])
	...
	print(data.frame(MCL_ID = unique_symbols[i], test.snp.mac, stringsAsFactors = FALSE))
	rm(test.snp.mac)
}
